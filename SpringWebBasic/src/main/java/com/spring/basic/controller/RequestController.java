...
//	@RequestMapping(value = "/request/basic01", method = RequestMethod.GET)
	@GetMapping("/basic01")
	public String req() {
		System.out.println("/request/basic01 요청 들어옴(GET방식)");
...
//	@RequestMapping(value = "/request/basic01", method = RequestMethod.POST)
	@PostMapping("/basic01")
	public String basic() {
		System.out.println("/request/basic01 요청 들어옴(POST방식)");
		return "request/req-ex01";
	}
	...
	//요청 URI주소가 같더라도 전송 방식에 따라 맵핑을 다르게 하기 때문에 같은 주소를 사용하는 것이 가능하다.
	//(GET -> 화면처리 할 때 많이 사용, POST -> 입력값 처리 할 때 많이 사용)
	
	/*
	 # 스프링에서 요청과 함께 전달된 데이터를 처리하는 방식
	 
	 1. 전통적인 jsp/servlet 방식의 파라미터 읽기 처리 방법
	  - HttpServletRequest 객체 활용(jsp에서 우리가 사용하던 방식)
