...
public class FreeBoardControllerTest {
	...
	/*
	 ...
	 - WebApplicationContext는 Spring에서 제공되는 servlet들을 사용할 수 있도록 정보를 저장하는 객체이다.
	 */
...
//	private FreeBoardController controller;
	...
	//MockMvc는 웹 어플리케이션을 서버에 배포하지 않아도 스프링 MVC 동작을 구현할 수 있는 가상의 구조를 만들어준다.
	private MockMvc mockMvc;
	
	@BeforeEach //테스트 시작시 다른 메서드 실행 전에 항상 먼저 구동되는 기능들을 선언
	public void setup() {
		//가상 구조를 세팅
		//스프링 컨테이너에 등록된 모든 빈과 의존성 주입까지 로드해서 사용 가능.
		this.mockMvc = MockMvcBuilders.webAppContextSetup(ctx).build();
		
		//테스트할 컨트롤러를 수동으로 주입. 하나의 컨트롤러 자체만 테스트를 진행할 때 이렇게 쓴다.
//		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	@Test
	@DisplayName("3번 글의 제목과 내용을 수정하는 요청을 post방식으로 전송하면 수정이 진행된 후 수정된 글의 상세포기 페이지로 이동할 것이다.")
